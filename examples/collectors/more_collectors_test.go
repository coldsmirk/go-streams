...
func Example_collectors_core_and_orderStats() {
	...
	largest := streams.CollectTo(streams.Of(1, 5, 3), streams.MaxByCollector[int](func(a, b int) int { return a - b })).Get()
	smallest := streams.CollectTo(streams.Of(1, 5, 3), streams.MinByCollector[int](func(a, b int) int { return a - b })).Get()
	fmt.Println(first, last, reduce)
	fmt.Println(joinFull)
	fmt.Println(count, sum, int(avg), largest, smallest)
